...
function ConfidenceChart({ probabilities, prediction }) {
  ...
  // Spring-animate bars toward target values
  useEffect(() => {
      ...
      >
        ...
        {animated.map((val, i) => {
          ...
          return (
            ...
            >
              {/* Glow shadow behind bar for winner */}
              {isWinner && barH > 2 && (
                <rect x={x - 2} y={y - 2} width={barW + 4} height={barH + 4} rx="5"
                  fill="url(#bar-grad-green)" opacity="0.25"
        ...
        {/* Hover crosshair dot on top of bar */}
        {hovered !== null && (() => {
...
function NetworkVisualization({ inputPixels, hidden1, hidden2, probabilities, prediction }) {
  ...
  // Continuous signal flow animation
  useEffect(() => {
    const animate = (ts) => {
  ...
  // Which connections glow when a node is selected
    ...
    <div className="net-wrap" onClick={() => setSelectedNode(null)}>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="xMidYMid meet"
        ...
        {connections.map((c, i) => {
          ...
          const baseOp = selectedNode
            ? (isHl ? 0 : 0.012)
            : 0.025 + c.strength * 0.15;
        ...
        {signalDots.map(dot => dot.opacity > 0.04 && (
          <circle key={dot.key}
            cx={dot.cx} cy={dot.cy} r={dot.isActive ? 3 : 2.2}
...
function App() {
  ...
  const predictDigit = async () => {
    const canvas = canvasRef.current;
    try {
      const response = await axios.post("https://digit-recognizer-api-916896635867.us-central1.run.app/predict", { image: canvas.toDataURL("image/png") });
    ...
    <div className="app">
      ...
      <div className="top-row">
        <div className="canvas-panel">
          <canvas ref={canvasRef} width={560} height={560} className="drawing-canvas large"
      ...
      <div className="network-section">
        ...
        <NetworkVisualization
          inputPixels={inputPixels} hidden1={hidden1}
          hidden2={hidden2} probabilities={probabilities} prediction={prediction} />
      </div>
    </div>
  );
}
